...
- 요지: JSON·CSV를 새 문서 종류로 추가. JSON은 유효할 때 2칸 들여쓰기로 정리하고
  ...
  파일 선택기·외부 VIEW/SEND·최근 파일 배지와 UTF-8/한국어 Excel 인코딩도 연결.
- 검증: `./gradlew test assembleRelease` 통과. Galaxy S21 Ultra에 동일 릴리스 키 APK
  업데이트 설치 후 외부 JSON VIEW, 앱 피커 CSV, CP949 한글 CSV, 넓은 표 가로 스크롤,
  고정 행 번호, 최근 파일 CSV 배지·재열기 확인. 앱 프로세스 FATAL/오류 로그 0건.
- ⚠️ 구조화 프리뷰 안전 한도: 원문 4MB, HTML 8MB, CSV 2,000행/20,000셀/행당
  200열. 초과분은 앞부분만 표시하며 기존 대용량 안내를 사용한다.
...
**"내 파일"에서 md/txt/json/csv/docx/html/pdf/이미지를 탭하면 열리는 오프라인 순수
...
- 뷰어 기능: md/txt 렌더, json 들여쓰기+원문 폴백, csv 표 렌더+UTF-8/한국어
  Excel 인코딩, docx 즉석 변환+렌더+"MD 저장"(마크다운+이미지 폴더
