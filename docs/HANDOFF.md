...
1. **앱 이름/아이콘** — 아직 "mdvault" 기본. 제품명·런처 아이콘 필요(출시 요건).
2. **targetSdk 35** — 현재 34. 올릴 때 SAF/미디어 접근 변화 점검.
3. 읽기 경험 개선은 사용자가 dogfooding 중 보고하는 순서대로.
   (글자 크기·읽기 위치·GIF/EXIF·MD 저장 이미지 포함은 2026-07-05 완료.)
   ※ 무효화된 백로그: SAF 목록 최적화(S4)·S5 Samsung Keyboard·S3 수동 확인은
     편집/볼트 제거로 더 이상 해당 없음.
